...
class DebugPaths:
    ...
    # New: per-sample emit/debug transcript outputs
    events_text_dir: str
...
def build_sample_from_dataset(
    ...
    if not tweet_text:
        raise ValueError("Dataset record is missing 'tweetText'.")
...
# ============================================================
# 4) Runtime assembly
#    All model-like components use the same llm object.
#    New: per-sample DebugSink is injected here.
# ============================================================
...
def build_runtime(
    *,
    ...
    max_write_refinement_attempts: int = 3,
) -> RuntimeBundle:
...
def serialize_evolver_run(evolver_run: Any) -> dict[str, Any]:
    return {
        ...
        "planner_records": [record.model_dump(mode="json") for record in evolver_run.planner_records],
        "actor_records": [record.model_dump(mode="json") for record in evolver_run.actor_records],
...
def write_outputs_module(
    ...
    if evolver_artifact is not None:
        ...
        # update in-memory store for this session
...
# ============================================================
# 9) Convenience loader for notebook usage
# ============================================================
